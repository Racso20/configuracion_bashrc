function exposePort(){
        ...
        if [ ${servicio^^} == "START" ]; then
                if ! [[ "$windows_port" =~ ^[0-9]+$ ]]; then
                        ...
                        powershell /C "netsh interface portproxy add v4tov4 listenport=$windows_port listenaddress=0.0.0.0 connectport=$linux_port connectaddress=$ip" 2>/dev/null
                ...
                if ! [[ "$windows_port" =~ ^[0-9]+$ ]]; then
                     ...
                     powershell /C "netsh interface portproxy del v4tov4 listenport=$windows_port listenaddress=0.0.0.0" 2>/dev/null
